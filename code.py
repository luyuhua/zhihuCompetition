# -*- coding: utf-8 -*-
"""
Created on Sun Jul  9 21:35:53 2017
...
fileName = './ieee_zhihu_cup/write.txt'
...
max_document_len = max([len(x.split(',')) for x in x_text])
...
input_y = tf.placeholder(tf.float32,[None,len(y_train[0])],name='input_y')
...
with tf.Session() as sess:
    ...
    for batch in batches:
        ...
        if i > i_tmp:
            ...
            # 优化模型
            sess.run([optimizer],feed_dict={input_x:x_batch,input_y:y_batch,dropout_keep_prob:0.5})
            ...
            # 每训练200次保存1次模型
            if(i%200 == 0):
                path = saver.save(sess, 'models/models_', global_step=i)
                print('Saved model chechpoint to {}'.format(path))
